...
% Convert y to a list of binary vectors
...
for i=1:m
    for k=1:num_labels
        J += -Y(i,k) * log(h2(i,k)) - (1 - Y(i,k)) * log(1 - h2(i,k));
    end
end

J = J / m;

%REGULARIZATION TIME AGAIN

reg_param = 0;

for j=1:size(Theta1)(1)
    for k=2:size(Theta1)(2)
        reg_param += (Theta1(j,k)) ^ 2;
    end
end

for j=1:size(Theta2)(1)
    for k=2:size(Theta2)(2)
        reg_param += Theta2(j,k) ^ 2;
    end
end

J += (lambda/(2*m)) * reg_param;
